...
class FedPSAuto():
    ...
    def train(self):
        ...
        for r in range(1, self.args.max_round + 1):
            ...
            if r % self.args.test_round == 0:
                ...
                # coefs
                for coef in self.model.shared_coefs:
                    self.logs["COEFS"].extend(
                        list(coef.detach().cpu().numpy().reshape(-1))
                    )
    ...
    def update_local(self, r, model, train_loader, test_loader):
        ...
        for t in range(n_total_bs + 1):
            ...
            # training global params
            model.train()
            logits = model.global_forward(batch_x)
