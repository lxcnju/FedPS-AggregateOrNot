...
class FedPSInter():
    ...
    def train(self):
        ...
        # inter test
        inter_accs = self.inter_test()
        print(inter_accs)
        self.logs["Inter_ACCS"] = list(inter_accs.reshape(-1))
    ...
    def inter_test(self):
        ...
        for client in self.clients:
            print(client)
            gmodel = self.model.shared_layers
            pmodel = self.private_models[client]

            if self.args.cuda:
                gmodel.cuda()
                pmodel.cuda()

            accs = self.inter_test_single(
                gmodel, pmodel, self.test_loaders[client]
            )
        ...
        all_accs = np.array(all_accs).mean(axis=0)
        return all_accs

    def inter_test_single(self, gmodel, pmodel, loader):
        xs = []
        ys = []
        for i, (batch_x, batch_y) in enumerate(loader):
            xs.append(batch_x)
        ...
        if xs.shape[0] > 512:
            inds = np.random.permutation(xs.shape[0])[0:512]
            xs = xs[inds]
            ys = ys[inds]
        ...
        with torch.no_grad():
            ...
            for alpha in np.linspace(0.0, 0.1, 11):
                fs = alpha * gfs + (1 - alpha) * pfs
                g_logis = gmodel[1](fs).softmax(dim=-1)
                p_logis = pmodel[1](fs).softmax(dim=-1)

                accs_beta = []
                for beta in np.linspace(0.0, 0.1, 11):
                    logis = beta * g_logis + (1 - beta) * p_logis
                    acc = count_acc(logis, ys)
                    accs_beta.append(acc)
                accs.append(accs_beta)
        accs = np.array(accs)
        return accs
